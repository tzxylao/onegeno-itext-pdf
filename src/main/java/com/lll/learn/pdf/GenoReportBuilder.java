...
public class GenoReportBuilder extends ReportBuilder {
    ...
    public GenoReportBuilder addContext() {
        java.util.List<ReportBean.CategoriesBean.ItemsBean> gaoLevel = new ArrayList<>();
        java.util.List<ReportBean.CategoriesBean> categories = reportBean.getCategories();
        for (ReportBean.CategoriesBean category : categories) {
            ...
            for (ReportBean.CategoriesBean.ItemsBean item : items) {
                ...
                if (!item.getLocked() && Integer.parseInt(item.getLevel()) >= 3) {
                    gaoLevel.add(item);
                }
            }
        }
        for (ReportBean.CategoriesBean.ItemsBean itemsBean : gaoLevel) {
            ...
            // 维生素矿物质
            boolean flag = "wss".equals(categoryCode) || "kwz".equals(categoryCode);
            if (flag) {
    ...
    private void addPersonalityTraits(ReportBean.CategoriesBean.ItemsBean itemsBean, String categoryCode) {
